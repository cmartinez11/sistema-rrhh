<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="font-bold text-2xl text-slate-800 leading-tight flex items-center">
                <svg class="w-7 h-7 me-3 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5m0 0h4m-4 0V11m0 0h4m-4 0H7m4 0V5"/></svg>
    ...
    <div class="py-8" x-data="{
        openCreate: false,
        openEdit: false,
        editArea: { id: null, nombre: '', descripcion: '' },
        setEdit(area) {
            this.editArea = { id: area.id, nombre: area.nombre, descripcion: area.descripcion || '' };
            this.openEdit = true;
        }
    }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <table class="w-full text-left text-sm text-slate-600">
                        <thead class="bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-6 py-3.5">Nombre del Área</th>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 text-slate-700">
                                        {{ $area->empleados_count }} empleado(s)
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <button @click="setEdit({{ json_encode($area) }})" class="p-2 text-cyan-600 hover:bg-cyan-50 rounded-lg transition" title="Editar Área">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <form action="{{ route('configuracion.areas.destroy', $area) }}" method="POST" class="inline" onsubmit="return confirm('¿Está seguro de eliminar el área \'{{ $area->nombre }}\'?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition" title="Eliminar Área">
            ...
            <div x-show="openCreate" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div @click.away="openCreate = false" class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
                    <div class="flex justify-between items-center border-b pb-3">
                        <h3 class="text-lg font-bold text-slate-800">Registrar Nueva Área</h3>
                        <button @click="openCreate = false" class="text-slate-400 hover:text-slate-600">&times;</button>
                    </div>
                    <form action="{{ route('configuracion.areas.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Nombre del Área *</label>
                            <input type="text" name="nombre" required class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-500 focus:ring-emerald-500" placeholder="Ej. Logística y Despachos">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Descripción</label>
                            <textarea name="descripcion" rows="3" class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-500 focus:ring-emerald-500" placeholder="Descripción opcional del área..."></textarea>
                        </div>
                        <div class="flex justify-end space-x-2 pt-4">
                            ...
                            <button type="submit" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-md">Guardar Área</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div x-show="openEdit" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div @click.away="openEdit = false" class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
                    <div class="flex justify-between items-center border-b pb-3">
                        <h3 class="text-lg font-bold text-slate-800">Editar Área</h3>
                        <button @click="openEdit = false" class="text-slate-400 hover:text-slate-600">&times;</button>
                    </div>
                    <form :action="'{{ url('configuracion/areas') }}/' + editArea.id" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Nombre del Área *</label>
                            <input type="text" name="nombre" x-model="editArea.nombre" required class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-500 focus:ring-emerald-500">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Descripción</label>
                            <textarea name="descripcion" x-model="editArea.descripcion" rows="3" class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-500 focus:ring-emerald-500"></textarea>
                        </div>
                        <div class="flex justify-end space-x-2 pt-4">
                            ...
                            <button type="submit" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-md">Actualizar Área</button>
                        </div>
                    </form>
                </div>
            </div>
