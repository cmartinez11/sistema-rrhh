    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="font-bold text-2xl text-slate-800 leading-tight flex items-center">
                ...
                Gestión de Documentos — Carta de No Renovación
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4">
                <div class="flex space-x-1 border-b border-slate-100 w-full lg:w-auto overflow-x-auto">
                    ...
                    <a href="{{ route('documentos.no-renovacion.index') }}" class="px-4 py-2.5 text-xs font-bold rounded-xl transition bg-cyan-600 text-white shadow-md whitespace-nowrap">
                        No Renovación
                    </a>
                ...
                <button @click="openUpload = true" class="w-full lg:w-auto px-5 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-xl shadow-md transition flex items-center justify-center whitespace-nowrap">
                    ...
                    + Emisión Carta No Renovación
                </button>
            </div>

            <!-- Filtros de Búsqueda -->
            <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
                <form method="GET" action="{{ route('documentos.no-renovacion.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-600 mb-1">Empleado</label>
                        <select name="empleado_id" class="w-full rounded-xl border-slate-300 text-sm focus:ring-cyan-500 focus:border-cyan-500">
                    ...
                    <div>
                        ...
                        <select name="estado_envio" class="w-full rounded-xl border-slate-300 text-sm focus:ring-cyan-500 focus:border-cyan-500">
                    ...
                    <div class="flex items-end space-x-2">
                        ...
                        <a href="{{ route('documentos.no-renovacion.index') }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-sm transition">
                            Limpiar
                        </a>
                    </div>
                </form>
                    ...
                    <table class="w-full text-left text-sm text-slate-600">
                        <thead class="bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-6 py-3.5">Asunto / Observación</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4 text-xs text-slate-500 max-w-xs truncate">
                                    {{ $doc->asunto_motivo ?? 'Término de Contrato' }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    <form action="{{ route('documentos.send', $doc) }}" method="POST" class="inline" onsubmit="return confirm('¿Desea enviar la Carta de No Renovación a {{ $doc->empleado->nombre_completo }}?');">
                                        @csrf
                                        <button type="submit" class="inline-flex items-center px-3 py-1.5 bg-cyan-50 hover:bg-cyan-100 text-cyan-800 rounded-lg text-xs font-semibold transition" title="Enviar por correo">
                                            <svg class="w-3.5 h-3.5 me-1 text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                            Enviar
                                        </button>
                                    </form>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-slate-400 italic">
                                    No se encontraron registros de Cartas de No Renovación.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
            ...
            <!-- Modal Registrar Carta No Renovación -->
            <div x-show="openUpload" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div @click.away="openUpload = false" class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
                    <div class="flex justify-between items-center border-b pb-3">
                        <h3 class="text-lg font-bold text-slate-800">Registrar Carta de No Renovación</h3>
                    ...
                    <form action="{{ route('documentos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <input type="hidden" name="tipo_documento" value="no_renovacion">

                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Empleado *</label>
                            <select name="empleado_id" required class="w-full text-sm rounded-xl border-slate-300 focus:border-cyan-500 focus:ring-cyan-500">
                        ...
                        <div>
                            ...
                            <input type="date" name="fecha_emision" value="{{ date('Y-m-d') }}" required class="w-full text-sm rounded-xl border-slate-300 focus:border-cyan-500 focus:ring-cyan-500">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Observación / Asunto (Opcional)</label>
                            <input type="text" name="asunto_motivo" class="w-full text-sm rounded-xl border-slate-300 focus:border-cyan-500 focus:ring-cyan-500" placeholder="Ej. Notificación de vencimiento de contrato de trabajo">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Archivo PDF (Máx. 10MB) *</label>
                            <input type="file" name="pdf" accept=".pdf" required class="w-full text-sm rounded-xl border-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100">
                        </div>
                        ...
                        <div class="flex justify-end space-x-2 pt-4 border-t border-slate-100">
                            ...
                            <button type="submit" class="px-5 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl text-sm font-bold shadow-md transition">Guardar Documento</button>
                        </div>
                    </form>
                </div>
            </div>
