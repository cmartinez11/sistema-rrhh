    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h2 class="font-bold text-2xl text-slate-800 leading-tight flex items-center">
                <svg class="w-7 h-7 me-3 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5m0 0h4m-4 0V11m0 0h4m-4 0H7m4 0V5"/></svg>
                Gestión de Documentos — Políticas de la Empresa
            </h2>
        </div>
    </x-slot>

    <div class="py-8" x-data="{ openUpload: false, despachoGeneral: false }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4">
                <div class="flex space-x-1 border-b border-slate-100 w-full lg:w-auto overflow-x-auto">
                    ...
                    <a href="{{ route('documentos.rit.index') }}" class="px-4 py-2.5 text-xs font-bold rounded-xl transition text-slate-600 hover:bg-slate-100 whitespace-nowrap">
                        Reglamento Interno (RIT)
                    </a>
                    <a href="{{ route('documentos.politicas.index') }}" class="px-4 py-2.5 text-xs font-bold rounded-xl transition bg-indigo-600 text-white shadow-md whitespace-nowrap">
                ...
                <button @click="openUpload = true" class="w-full lg:w-auto px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition flex items-center justify-center whitespace-nowrap">
                    ...
                    + Emisión Políticas de la Empresa
                </button>
            </div>

            <!-- Filtros de Búsqueda -->
            <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
                <form method="GET" action="{{ route('documentos.politicas.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-600 mb-1">Empleado</label>
                        <select name="empleado_id" class="w-full rounded-xl border-slate-300 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div>
                        ...
                        <select name="estado_envio" class="w-full rounded-xl border-slate-300 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div class="flex items-end space-x-2">
                        ...
                        <a href="{{ route('documentos.politicas.index') }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-sm transition">
                            Limpiar
                        </a>
                    </div>
                </form>
                    ...
                    <table class="w-full text-left text-sm text-slate-600">
                        <thead class="bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-6 py-3.5">Asunto / Política</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4 text-xs text-slate-600 max-w-xs truncate">
                                    {{ $doc->asunto_motivo ?? 'Políticas de la Empresa - PLASTICOS FENIX' }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    <form action="{{ route('documentos.send', $doc) }}" method="POST" class="inline" onsubmit="return confirm('¿Desea enviar las Políticas de la Empresa por correo a {{ $doc->empleado->nombre_completo }}?');">
                                        @csrf
                                        <button type="submit" class="inline-flex items-center px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 rounded-lg text-xs font-semibold transition" title="Enviar por correo">
                                            <svg class="w-3.5 h-3.5 me-1 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                            Enviar
                                        </button>
                                    </form>

                                    <form action="{{ route('documentos.destroy', $doc) }}" method="POST" class="inline" onsubmit="return confirm('¿Está seguro de eliminar este registro?');">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-slate-400 italic">
                                    No se encontraron registros de Políticas de la Empresa.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
            ...
            <!-- Modal Registrar Políticas de la Empresa -->
            <div x-show="openUpload" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div @click.away="openUpload = false" class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
                    <div class="flex justify-between items-center border-b pb-3">
                        <h3 class="text-lg font-bold text-slate-800">Registrar Políticas de la Empresa</h3>
                    ...
                    <form action="{{ route('documentos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <input type="hidden" name="tipo_documento" value="politicas_empresa">

                        <!-- Opción Despacho General vs Empleado Individual -->
                        <div class="p-3 bg-indigo-50/60 rounded-xl border border-indigo-100 space-y-2">
                            <label class="flex items-center space-x-3 cursor-pointer">
                                <input type="checkbox" name="despacho_general" value="1" x-model="despachoGeneral" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs font-bold text-indigo-900 uppercase">Despacho General (A todo el personal activo)</span>
                            </label>
                            <p class="text-[11px] text-indigo-700">Al marcar esta casilla, el PDF subido se registrará para todos los empleados activos en el sistema.</p>
                        </div>

                        <div x-show="!despachoGeneral">
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Empleado Individual *</label>
                            <select name="empleado_id" :required="!despachoGeneral" class="w-full text-sm rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Fecha de Emisión / Entrega *</label>
                            <input type="date" name="fecha_emision" value="{{ date('Y-m-d') }}" required class="w-full text-sm rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Asunto / Título de la Política (Opcional)</label>
                            <input type="text" name="asunto_motivo" class="w-full text-sm rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Ej. Política de Seguridad y Salud en el Trabajo 2026">
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-600 uppercase mb-1">Archivo PDF (Máx. 10MB) *</label>
                            <input type="file" name="pdf" accept=".pdf" required class="w-full text-sm rounded-xl border-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                        </div>
                        ...
                        <div class="flex justify-end space-x-2 pt-4 border-t border-slate-100">
                            ...
                            <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-md transition">Guardar y Emitir</button>
                        </div>
                    </form>
                </div>
            </div>
